...
- `SandboxMigrationService` — **macOS is not sandboxed** (`app/macos/Runner/*.entitlements` deliberately omit `com.apple.security.app-sandbox`): the sandbox rewrote `HOME` to `~/Library/Containers/com.thangnm.yourssh/Data`, where `~/Downloads`, `~/Desktop` and `~/.ssh` are only symlinks that raise `EPERM` — unusable for an SSH/SFTP client, and Apple ships no entitlement that covers Desktop/Documents (the exception entitlements that would, `keychain-access-groups` and `temporary-exception.*`, are restricted: an ad-hoc-signed build claiming one is SIGKILLed at launch, and this app ships unsigned via DMG/zip, not the Mac App Store). Folder access is now governed by macOS privacy (TCC) prompts, explained by the `NS*FolderUsageDescription` strings in `Info.plist`. This service is the one-time carry-over run from `main()` before any provider reads prefs: `parseXmlPlist` (pure) reads the container's prefs domain via `plutil -convert xml1`, `planPrefsMigration` (pure) drops keys the new domain already has, and the audit database + `~/.yourssh/plugins` are copied file-by-file, never overwriting. Keys and recordings are deliberately *not* copied — prefs hold their absolute container paths and an unsandboxed app can still read the container, so copying would only duplicate private key material. Every step is fail-soft and the `macos_sandbox_migration_done` marker is written either way
...
- `fs_error.dart` — `describeFileSystemError` maps a `dart:io` failure to a sentence the UI can show: EPERM/EACCES (macOS privacy denial, POSIX mode) becomes a "grant access in System Settings → Privacy & Security" hint, ENOENT becomes "no longer exists", anything else keeps its own message; used by the local file panel's list/create/rename/delete/chmod paths instead of a raw `PathAccessException` dump
...
Passwords use a secure-first strategy: `StorageService` writes to `FlutterSecureStorage` (Keychain on macOS, Credential Manager on Windows) first, and on success purges any stale plaintext copy from `SharedPreferences` (left over from prior fallbacks). Only if secure storage throws does it fall back to writing plaintext to `SharedPreferences`. Reads prefer secure storage, fall back to prefs. Keys: `pw_<hostId>` for passwords, `pp_<keyId>` for key passphrases, `sync_passphrase` for the optional sync passphrase. SSH certificate paths are stored alongside `SshKeyEntry` (file paths, not secrets). On macOS the store is pinned to the **legacy file Keychain** (`StorageService.macOsKeychainOptions` = `MacOsOptions(accountName: 'yourssh', useDataProtectionKeyChain: false)`): `flutter_secure_storage` defaults to the data-protection Keychain, which needs a keychain-access-group entitlement no ad-hoc-signed build can carry, so every `SecItemAdd` returned `-34018` and every secret silently took the plaintext prefs path. The legacy Keychain accepts the same items unsandboxed with no entitlement (changing the ad-hoc signature between versions can make macOS re-prompt for access). `migratePlaintextSecrets()` runs from `main()` on every launch and moves any secret still sitting in prefs (`pw_`/`sudopw_`/`pp_` prefixes + `sync_passphrase`, classified by `isSecretPrefsKey`) into the Keychain — write, **read back to confirm**, then remove the prefs copy, so a failed write never destroys the only copy of a password; keys that can't be moved stay put and are retried next launch. The remaining fallback is no longer silent: `plaintextSecretKeys` / `hasPlaintextSecrets` drive the Settings → Security section (with a Retry button) and `onPlaintextFallback` raises an `AppNotificationType.insecureStorage` item in the notification bell.
