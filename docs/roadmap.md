...
> Current version: 0.1.31 · updated: 2026-06-06 (release 0.1.31: connection chain editor, jump-host fix on auto-connect paths)
...
- **Multi-hop jump chain** — _visual chain editor shipped single-hop (0.1.31)._ Remaining: multiple hops (bastion → bastion → target) — `Host.jumpHostId` becomes a list, `SshService` dials recursively, chain UI re-enables Add a Host with a hop already set.
